...
export default function AdminDownloads({ userId }: { userId: number }) {
  ...
  const fetchDownloads = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || 'Error al cargar descargas');
      }
    } catch (err) {
  ...
  const insertBBCode = (ref: React.RefObject<HTMLTextAreaElement>, open: string, close: string, setter: (val: string) => void) => {
  ...
  const insertPromptTag = (ref: React.RefObject<HTMLTextAreaElement>, tagType: 'color' | 'size' | 'img', setter: (val: string) => void) => {
    let val = '';
