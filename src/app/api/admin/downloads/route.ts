...
export async function POST(request: Request) {
  try {
    ...
    console.error('Admin downloads POST error:', error);
    return NextResponse.json({ error: 'No se pudo crear la descarga (verificar que el slug sea único)' }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    return NextResponse.json({ error: 'No se pudo actualizar la descarga' }, { status: 500 });
  }
}
